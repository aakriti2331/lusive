...
class Admin extends CI_Controller
{
    ...
    function vendor_add($para1 = "", $para2 = "")
    {
        $safe = 'yes';
        $char = '';
        ...
        if ($para1 == "add_info") {
            ...
            $this->form_validation->set_rules('gstno','GST Number');
            $this->form_validation->set_rules('licence','Licence Number');
          ...
          {
               $profile_image = $this->upload_single_image_report($files);
          }

          $aadhar_image='';
          $files = $_FILES['adharimg'];
          if(!empty($_FILES['adharimg']['name']))
          {
               $aadhar_image = $this->upload_single_image_report($files);
          }

          $pancard_image='';
          $files = $_FILES['pancardimg'];
          if(!empty($_FILES['pancardimg']['name']))
          {
               $pancard_image = $this->upload_single_image_report($files);
          }
                    ...
                    if ($this->crud_model->get_settings_value('general_settings', 'captcha_status', 'value') == 'ok') {
                        ...
                        if ($response['success']) {
                            ...
                            $data['license'] = $this->input->post('licence');
                            $data['adharimg'] = $aadhar_image;
                            $data['pancardimg'] = $pancard_image;
                            ...
                            if ($this->input->post('password1') == $this->input->post('password2')) {
                                ...
                                if ($this->email_model->account_opening('vendor', $data['email'], $password) == false) {
                                    ...
                                    $date = $this->session->set_flashdata('message', 'Registration successfully');
                        ...
                        if ($this->input->post('password1') == $this->input->post('password2')) {
                            ...
                            if ($this->email_model->account_opening('vendor', $data['email'], $password) == true) {
                                if ($this->email_model->vendor_reg_email_to_admin($data['email'], $password) == false) {
                                    ...
                                    $date = $this->session->set_flashdata('message', 'Vendor add successfully');
                                    redirect(base_url() . 'admin/vendor/', 'refresh');
                                }
                            } else {
                                $date = $this->session->set_flashdata('message', 'Vendor add successfully');
        ...
        //redirect(base_url() . 'admin/doctor_list/', 'refresh');
    }
    ...
    function repair()
    {
        ...
        //     redirect(base_url() . 'admin');
        // }
        $page_data['page_name'] = "repair";
        $page_data['repair']  = $this->Api_Model->selectrepair('repair');
        $this->load->view('back/index', $page_data);
    }
    ...
    function notification()
    {
        ...
        $page_data['notification'] = $this->Api_Model->selectnotification('notification');
        $this->load->view('back/index', $page_data);
    }
    ...
    public function update_notification()
    {
        ...
        if (!empty($_FILES['url']['name'])) {
            $url = $this->upload_single_image($files);
            $updateimg = $this->Api_Model->update('notification', array('image' => $url), array('id' => $id));
        }
        ...
        $update = $this->Api_Model->update('notification', $dataArray, array('id' => $id));
        if (!empty($update)) {
            $this->session->set_flashdata('message', 'Successfully Notification Updated.');
            redirect('Admin/notification');
        } else {
            $this->session->set_flashdata('message', 'Not Successfully Notification Type Updated.');
            redirect('Admin/notification');
        }
        $date = $this->session->set_flashdata('message', 'Update successfully');
    }

    public function sendnotification($id)
    {
        $notificationn = $this->Api_Model->selectrow('notification', array('id' => $id));
        if (!empty($notificationn->unique_id)) {
            $expdata = explode(',', $notificationn->unique_id);
            $valuedata = array();
            $uniqueval = array();
            foreach ($expdata as $dataval) {
                $uniqueval[] = $this->Api_Model->selectrow('user', array('user_id' => $dataval));
            }

            foreach ($uniqueval as $row) {
                $title = $notificationn->title;
                $token = $row->reg_id;
                $body = $notificationn->description;
                $this->send_pushnotification($token, $title, $body);
            }
            echo '<script> location.replace("' . base_url() . 'Admin/notification") </script>';
            //redirect('Admin/notification');
        }
    }


    public function send_pushnotification($token, $title, $body)
    {
        $url = "https://fcm.googleapis.com/fcm/send";
        $token = $token;
        $serverKey = 'AAAAuOqYOxA:APA91bEMusJUSmR6R8zc9WbZTtDAlM_4mk2vt9JbzcCOl6wMK-WPNtD2_c8iZsS3gK7JYUkgAgaR2LQGLGDWaSvSTdnKvXMILzImjPzI4JI97aiDXq02gXFssXm9aVIp4qhTEo56KpeW';
    ...
    {
        $con = array('id' => $id);
        $delete = $this->Api_Model->delete('notification', $con);
        $this->session->set_flashdata('message', 'Successfully Delete Notification.');
        redirect('Admin/notification');
    }

    public function addnotification()
    {
        $page_data['page_name'] = "addnotification";
        $page_data['users'] = $this->Api_Model->select('user');
        $this->load->view('back/index', $page_data);
    }
    function editnotification($id)
    {

        $page_data['page_name']   = "edit_notification";
        $page_data['notification_info'] = $this->Api_Model->selectrow('notification', array('id' => $id));
        $page_data['users'] = $this->Api_Model->select('user');
        $this->load->view('back/index', $page_data);
    }
    function registration($para1 = "", $para2 = "")
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $insert = $this->Api_Model->insert('notification', $dataArray);
            if (!empty($insert)) {
                $this->session->set_flashdata('message', 'Successfully Notification Added.');
                redirect('Admin/notification');
            }
        }
    ...
    function rejext_product()
    {
        if ($this->input->post('submit') != "") {
            /*  echo "dfg";
            print_r($this->input->post());
            //exit; */
            $new_pro_id          = $this->input->post('new_pro_id');
            $data['rejection_reason']   = $this->input->post('rejection_reason');
            $data['status']   = 0;
            $this->db->where('product_id', $new_pro_id);
    ...
    function product($para1 = '', $para2 = '', $para3 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            /* if ($search) {
                ...
                $result_vendor = $this->db->get()->result_array();
                foreach ($result_vendor as $id) :
                    $ids[] = $id['vendor_id'];
                endforeach;
                echo "<pre>";
                print_r($ids);
                exit;
                //echo "<script>console.log('oksan'); </script>";
            ...
            foreach ($products as $row) {
                ...
                if ($row['discount'] != '') {
                    $res['discount']  = $row['discount'] . ' ' . ucfirst($row['discount_type']);
                } else {
                    $res['discount']  = '-';
                }
                //add html for action
            ...
            if ($para3 == 'true') {
                ...
                echo "<script> document.getElementById('new_pro_id').value = $product;</script>";
                echo "<script> $('#mynewModal').modal('show');</script>";
                exit;
                $data['status'] = '0';
            }
            $this->db->where('product_id', $product);
    ...
    function low_inventry($para1 = '', $para2 = '')
    {
        if (!$this->crud_model->admin_permission('stock')) {
            redirect(base_url() . 'admin');
        }
        ...
        elseif ($para1 == 'add_stock') {
            ...
            $this->load->view('back/admin/product_stock_add', $data);
             ...
             $this->db->order_by('product_id', 'desc');
            $this->db->where('download=', NULL);
            $this->db->where('current_stock<=',10);
            $page_data['all_product'] = $this->db->get('product')->result_array();
            $this->load->view('back/admin/low_inventry_list', $page_data);
        } elseif ($para1 == 'add') {
            $this->load->view('back/admin/stock_add');
        } elseif ($para1 == 'destroy') {
            $this->load->view('back/admin/stock_destroy');
            ...
            $page_data['page_name'] = "low_inventry";
            $page_data['all_product'] = $this->db->get_where('product', array('current_stock<=' => 10))->result_array();
            $this->load->view('back/index', $page_data);
        }
    }
    ...
    function vendor_list($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('vendor') || $this->crud_model->get_type_name_by_id('general_settings', '58', 'value') !== 'ok') {
            redirect(base_url() . 'admin');
        }
        ...
        if ($para1 == 'delete') {
            ...
            $this->db->order_by('vendor_id', 'desc');
            $page_data['all_vendors'] = $this->db->get('vendor')->result_array();
            $this->load->view('back/admin/vendor_list_active', $page_data);
        }
         else if ($para1 == 'view') {
            $page_data['vendor_data'] = $this->db->get_where('vendor', array(
                'vendor_id' => $para2
            ...
            $page_data['page_name'] = "vendor_active";
            $page_data['all_vendors'] = $this->db->get('vendor')->result_array();
            $this->load->view('back/index', $page_data);
        }
    }
    ...
    //         'value' => $this->input->post('vendor_commission'),'vendorid'=>$this->input->post('vendorid')
    //     ));
    //     recache();
    ...
    function set_commission($para1 = '', $para2 = '', $para3 = '', $para4 = '')
    {
        ...
        $vendor = $this->crud_model->selectrow('business_settings', array('vendorid' => $vendorid));
        if (!empty($vendorid)) {
            if (empty($vendor)) {
                $save = array(
                ...
                $insert = $this->crud_model->insert('business_settings', $save);
                recache();
            } else {
                $updatedata = array(
                    'type' => 'commission_amount',
                ...
                $update = $this->crud_model->update('business_settings', $updatedata, array('vendorid' => $vendorid));
                recache();
            }
        } else {
            $updatedata = array(
            ...
            $update = $this->crud_model->update('business_settings', $updatedata, array('type' => 'commission_amount', 'vendorid' => null));
            recache();
        }
    }
    ...
    function upload_single_image_report($files){
        ...
        if(!empty($files['name'])){
            ...
            $_FILES['file']['name']     = time().$files['name'];
                ...
                if (!file_exists ('assets/uploads/report_image/'))
                {

                        mkdir('assets/uploads/report_image/',0777,true);  
