...

{
...

    {
        ...
        if(!empty($carted))
        {
            ...
            {
                $cartmatch= $this->Api_Model->select('cart',array('user_id'=>$uid,'id'=>$row['id'],'status'=>0));
                if(empty($cartmatch))
                {
               $datacart = array(
                'user_id'=>$uid,
        ...
        $dataa = $this->db->get_where('cart', array('user_id' =>$uid,'status'=>0))->result_array();
        if(!empty($dataa))
        {
    ...
    public function repair($para1 = "")
    {
    		$page_data['category']= $this->Api_Model->select('category');
            $page_data['page_name'] = "others/repair";

            $page_data['asset_page'] = "repair";
    ...
    public function add_repair()
    {
        $this->load->library('form_validation');
            ...
            if($this->form_validation->run() === FALSE)
            {
                ...
                $this->session->set_flashdata('message', 'Not successfully Repair query.');
                $page_data['category']= $this->Api_Model->select('category');
            $page_data['page_name'] = "others/repair";
            ...
            else
            {
                ...
                $insert = $this->Api_Model->insert('repair',$repairdata);
                if(!empty($insert))
                {
                     $repair  = $this->Api_Model->selectrepairrow('repair',array('repair_id'=>$insert));
                    $msg = "<html>
                            <head>
                                <title>Repair mail</title>
                            </head>

                            <body>
                                <h2>Name: ".$this->input->post('firstname')."</h2>
                                <h2>Mobile: ".$this->input->post('mobile')."</h2>
                                <h2>Category Name: ".$repair->category_name."</h2>
                                <h2>Brand Name: ".$repair->name."</h2>
                                <h2>Modal: ".$repair->sub_category_name."</h2>
                                <h2>Location: ".$this->input->post('location')."</h2>
                     ...
                     $config = Array(
                      ...
                      'smtp_user' => 'kundan.adsandurl@gmail.com',

                      'smtp_pass' => 'kundan19935',
...
$this->email->from('py995898@gmail.com','Admin'); // change it to yours
$this->email->to('mail4upky@gmail.com');// change it to yours
$this->email->subject('Thank You');
$this->email->message($msg);
$this->email->send();
                        $this->session->set_flashdata('message', 'Successfully repair query.');
    ...
    public function repair_brand()
    {
    	$category_id=$this->input->post('category_id');
    	$category= $this->Api_Model->select('category',array('category_id'=>$category_id));
    	if(!empty($category))
    	{
    		echo '<option selected="true" disabled="disabled">-Select Brand-</option>';
    	foreach ($category as $row)
    	{
    		$databrands=explode(':::', $row->data_brands);
    		$databrandsnew=implode(';;;;;;', $databrands);
    		$databrandsnew=explode(';;;;;;', $databrandsnew);
    		$cc=count($databrandsnew);
    		for ($i=0;$i<=$cc;$i++)
    		{
    			if(is_numeric($databrandsnew[$i]))
    			{
    				$brand= $this->Api_Model->selectrow('brand',array('brand_id'=>$databrandsnew[$i]));
    				if(!empty($brand))
    				{
    ...
    public function repair_modal()
    {
        $brand_id=$this->input->post('brand_id');
        $brand='"' . $brand_id . '"';
        $branddata= brandid_by_subcategory($brand);

        if(!empty($branddata))
        {   echo '<option selected="true" disabled="disabled">-Select Modal-</option>';
                ...
                if ($safe == 'yes') {
                    ...
                    if ($this->crud_model->get_settings_value('general_settings', 'captcha_status', 'value') == 'ok') {
                        ...
                        if ($response['success']) {
                            ...
                            {
                                $adharimg = $this->upload_single_image($files);
                            }
                            ...
                            {
                                $pancardimg = $this->upload_single_image($files);
                            }
                            ...
                            if ($this->input->post('password1') == $this->input->post('password2')) {
                                ...
                                $data['password_show'] = $this->input->post('password1');
    ...
    function cart($para1 = '', $para2 = '', $para3 = '', $para4 = '')
    {
        ...
        if ($para1 == "add") {
            ...
            if(!empty($uid))
            {
                $cartmatch= $this->Api_Model->select('cart',array('user_id'=>$uid,'id'=>$para2,'status'=>0));
                if(empty($cartmatch))
                {
                $datacart = array(
                'user_id'=>$uid,
            ...
            foreach ($carted as $items) {
                if ($items['rowid'] == $para2) {
                    if ($current_quantity >= $para3) {
                        ...
                        if(!empty($uid))
                        {
                        $update = $this->Api_Model->update('cart',array('qty'=>$para3),array('user_id'=>$uid,'id'=>$items['id'],'status'=>0));
                    }
                    } else {
                        ...
                        if(!empty($uid))
                        {
                        $update = $this->Api_Model->update('cart',array('qty'=>$current_quantity),array('user_id'=>$uid,'id'=>$items['id'],'status'=>0));
                    }
                    }
                } else {
                    $data = array(
                    ...
                    if(!empty($uid))
                        {
                        $update = $this->Api_Model->update('cart',array('qty'=>$items['qty']),array('user_id'=>$uid,'id'=>$items['id'],'status'=>0));
                    }
                }
                $this->cart->update($data);
            }
        ...
        if ($para1 == "remove_one") {
            ...
            if(!empty($uid))
            {
                ...
                $delete=$this->Api_Model->delete('cart',$datacart);
...
public function checkout($id) {
    ...
    private function get_curl_handle($payment_id, $amount)  {
        ...
        $key_id = 'rzp_test_1uR2uo44iso36u';
        $key_secret = 'ehBr4l9pe3FEKe6XotQDDOus';
        ...
        foreach ($carted as $row)
            {
                ...
                $update = $this->Api_Model->update('cart',array('status'=>1),array('user_id'=>$uid,'id'=>$row['id']));
            }
            } 
    $carted=$this->session->userdata('cart_contents');
        if (count($carted) <= 0) {
            redirect(base_url() . 'home/', 'refresh');
        }
        ...
        if (!empty($this->input->post('razorpay_payment_id')) && !empty($this->input->post('merchant_order_id'))) {
            ...
            if ($success === true) {
                ...
                if (!$order_info['merchant_order_id']) {
                    $this->session->set_userdata('couponer', '');
                        if ($this->session->userdata('user_login') == 'yes') {
                ...
                $this->session->set_flashdata('alert', 'unsuccessful_razor');
                        redirect(base_url() . 'home/cart_checkout/', 'refresh');
            }
        } else {
    ...
    {
       // print_r($this->input->post('payment_type'));die;
        $carted = $this->cart->contents();
        if (count($carted) <= 0) {
            redirect(base_url() . 'home/', 'refresh');
        }
        ...
        }  else if ($this->input->post('payment_type') == 'online') { // razor pay
                //echo "<pre>"; print_r($this->input->post('payment_type')); die;
                if ($para1 == 'go') {
                    $data['product_details'] = $product_details;
                    $data['shipping_address'] = json_encode($_POST);
                    $data['vat'] = $vat;
                    ...
                    $this->db->insert('sale', $data);
                     $sale_id = $this->db->insert_id();
                    if ($this->session->userdata('user_login') == 'yes') {
                        $data1['buyer'] = $this->session->userdata('user_id');

                        $data['get_user_details'] = get_user_details($this->session->userdata('user_id'));
                    ...
                    $data1['sale_code'] = date('Ym', $data['sale_datetime']) . $sale_id;
        ...
        foreach ($carted as $row)
            {
                ...
                $update = $this->Api_Model->update('cart',array('status'=>1),array('user_id'=>$uid,'id'=>$row['id']));
                    ...
                    $this->session->set_userdata('sale_id', $sale_id);
                    // FOR PAYMENT GAteway
                    $data['return_url'] = base_url().'home/return_url';
                    $data['surl'] = base_url().'home/success_payments';;
                    $data['furl'] = base_url().'home/failed_payments';;
                    $data['currency_code'] = 'INR';

                     $user_id = $this->session->userdata('user_id');

                     $this->load->view('front/shopping_cart/payment', $data);
                }
            ...
            echo $this->input->post('payment_type');die;
            if ($para1 == 'go') {
        ...
        else if ($this->input->post('payment_type') == 'razor') {
            echo $this->input->post('payment_type');die;
            if ($para1 == 'go') {
                if (isset($_POST['stripeToken'])) {
...
public function return_url() 
    {   
        if (!empty($this->input->post('razorpay_payment_id')) && !empty($this->input->post('merchant_order_id'))) {
            ...
            $error = '';
            try {                
                    $ch = $this->get_curl_handles($razorpay_payment_id, $amount);
                        //execute post
                    $result = curl_exec($ch);
                    $http_status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                    if ($result === false) {
    ...
    private function get_curl_handles($payment_id, $amount) 
    {
        $url = 'https://api.razorpay.com/v1/payments/'.$payment_id.'/capture';
        $key_id = "rzp_test_DnQ31lycSRBKGR";
        $key_secret = "qvZCCazH6NyacTGhOW3d3bbC";
        $fields_string = "amount=$amount";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_USERPWD, $key_id.':'.$key_secret);
