 ...
 */
	...
	//GET CURRENCY
	if ( ! function_exists('get_prescription'))
	{
		function get_prescription($id=''){
			$CI=& get_instance();
			$CI->db->select('is_prescription');    
	        $CI->db->from('product');
	        $CI->db->where('product_id',$id);
	        $CI->db->where('is_prescription','1');
	        $num_rows = $CI->db->get()->num_rows();
...
if ( ! function_exists('get_user_details'))
{
    function get_user_details($id)
    {
        $CI=& get_instance();
        $CI->load->database();
        $result = $CI->db->get_where('user',array('user_id'=>$id));
       return $result->row();
